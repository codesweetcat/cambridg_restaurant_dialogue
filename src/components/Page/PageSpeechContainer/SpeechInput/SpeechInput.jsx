...
const SpeechInput = () => {
  ...
  const task_id =
    mturkArray && mturkArray.length == 3
      ? mturkArray[1].split('=')[1]
      : undefined
  const mturk_user_id =
    mturkArray && mturkArray.length == 3
      ? mturkArray[0].split('=')[1]
      : undefined
  const exp_cond =
    mturkArray && mturkArray.length == 3
      ? mturkArray[2].split('=')[1]
      : undefined
      ...
      getDialogueJsonPhp(_endpointHubPHP, jsonData).then((res) => {
        ...
        if (res.error) {
          ...
          if (res.error_display) setErrorDescription(res.error_description)
            ...
            'getDialogueUtt(): hungup but system says success=false'
          ) {
            setFinalEvaluation(false)
          } else {
            setErrorDescription(
              'Server is down. Could you please try again later...',
            )
          }
        } else if (res.data.system_utt) {
          //sequence exchange
          if (
            (transcript.toLowerCase().trim() === 'goodbye' &&
              res.data.success) ||
            (res.data.intent &&
              res.data.intent === 'returnGoodbye' &&
              res.data.success) ||
            res.data.utt_number >= _maxOfUtterTurnsEnableTokenBtn
          ) {
            //isUtt && res.data.intent && res.data.intent === 'returnGoodbye' ||
          ...
          const newResponseStorage = !responseStorage
            ? [res]
            : [...responseStorage, res]
          ...
          <div className="input-group">
            <div className="container">
              <form id="search-form">
                ...
                />
                {recognition && (
                  <button type="button" onClick={micBtnClick}>
                    <i
                      className={
                        isRecording
                          ? 'fa fa-microphone-slash red-slash'
                          : 'fa fa-microphone'
                      }
                    ></i>
                  </button>
